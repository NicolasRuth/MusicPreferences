...
full_dat <- merge(genre_dat, Musical_collaboration[,c(28:31,163:167,174:178)], all=T, by.x = "data.id", by.y= "id", all.x = T)
full_dat$STOMP_genre <- as.factor(full_dat$STOMP_genre)
full_dat$MUSIC_genre <- as.factor(full_dat$MUSIC_genre)

library(glmertree)
m_1 <- lmertree(MUSIC_rating ~ STOMP_rating | data.id | age + gender + education +STOMP_genre+TIPI_Extroversion + TIPI_Agreeableness+TIPI_Emotional.Stability+TIPI_Conscientiousness+TIPI_Openness+Emotional.Mean+Cognitive.Mean+Background+EQ.score.mean+SQ.score.mean, data = full_dat)
